...
namespace DigitalRise
{
	public class DynamicEffectsSource : IEffectsSource
	{
		...
		private EffectSource AddSourceFile(string file)
		{
			...
			DR.LogInfo($"Added effect source file '{file}'");
			...
			foreach (Match match in matches)
			{
				...
				DR.LogInfo($"'{file}' depends on '{includeFile}'");
		...
		public Effect GetEffect(Assembly assembly, string name, Dictionary<string, string> defines)
		{
			try
			{
				...
				{
					DR.LogInfo($"Compiled version '{binaryPath}' exist");
				}
				else
				{
					DR.LogInfo($"Compiled version '{binaryPath}' doesn't exist");
				}

				byte[] effectData;
				if (binaryVersionExists)
				{
					DR.LogInfo("Using compiled version of the effect");
				...
				var effect = new Effect(DR.GraphicsDevice, effectData);
		...
		public Effect UpdateEffect(Effect effect)
		{
			...
			try
			{
				...
				DR.LogInfo($"Writing '{binaryPath}'");
				File.WriteAllBytes(binaryPath, effectData);

				var newEffect = new Effect(DR.GraphicsDevice, effectData)
				{
					Tag = effectInfo
				};
