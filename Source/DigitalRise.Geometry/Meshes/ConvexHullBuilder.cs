    ...
    public void Grow(IEnumerable<Vector3> points, int vertexLimit, float skinWidth)
    {
      ...
      #region ----- Convert to Unit Cube -----
      {
        ...
        for (int i = 0; i < pointList.Count; i++)
          pointList[i] = toUnitCube.TransformPosition(pointList[i]);
      }
      #endregion
...
//#endif
      ...
      if (_mesh.Faces.Count > 2 && (!Numeric.IsZero(skinWidth) || _mesh.Vertices.Count >= vertexLimit))
      {
        ...
        var skinWidthScale = toUnitCube.TransformDirection(new Vector3(skinWidth));
      ...
      foreach (var v in _mesh.Vertices)
        v.Position = fromUnitCube.TransformPosition(v.Position);
    }
    ...
    private void GrowConvex(Vector3 point, DcelFace face)
    {
      ...
      // (e.g. DigitalRise.Physics.Tests.MassTest.ScaledConvexMass()).
