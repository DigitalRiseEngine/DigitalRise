...
namespace DigitalRise.Geometry.Partitioning
{
  partial class AdaptiveBoundingBoxTree<T>
  {
    ...
    /// </para>
    /// <para>
    /// The <see cref="AdaptiveBoundingBoxTree{T}"/> uses a mixed approach: It starts with a top-down
    ...
    private void FullRefit(Node node)
    {
      ...
      else
      {
        ...
        if (!node.IsValid)
        {
          ...
          leaf.IsActive = false;
          leaf.BoundingBox = GetBoundingBoxForItem(leaf.Item);
          node.BoundingBox = leaf.BoundingBox;
          ...
          for (int index = 1; index < numberOfLeaves; index++)
          {
            ...
            leaf.IsActive = false;
            leaf.BoundingBox = GetBoundingBoxForItem(leaf.Item);
            node.BoundingBox.Grow(leaf.BoundingBox);
          }
        }
        else
        {
          ...
          node.BoundingBox = Microsoft.Xna.Framework.BoundingBox.CreateMerged(node.LeftChild.BoundingBox, node.RightChild.BoundingBox);
    ...
    private static void InvalidateSubtree(Node node)
    {
      ...
      node.Leaves = DigitalRise.ResourcePools<Node>.Lists.Obtain();
    ...
    private static bool IsDegenerate(Node node)
    {
      ...
      float volumeParent = node.BoundingBox.Volume();
      float volumeLeftChild = node.LeftChild.BoundingBox.Volume();
      float volumeRightChild = node.RightChild.BoundingBox.Volume();
    ...
    private void AddLeaf(T item)
    {
      ...
      leaf.BoundingBox = GetBoundingBoxForItem(item);
      leaf.Item = item;
      _leaves.Add(leaf);
    ...
    private void AddLeaf(Node node, Node leaf)
    {
      ...
      if (node.IsValid)
      {
        ...
        int selection = BoundingBoxTreeHelper.SelectClosest(leaf.BoundingBox, node.LeftChild.BoundingBox, node.RightChild.BoundingBox);
        if (selection == 0)
        {
    ...
    private static void RecomputeBoundingBox(Node node)
    {
      Debug.Assert(!node.IsLeaf);
      ...
      {
        node.BoundingBox = node.Leaves[0].BoundingBox;
        int numberOfLeaves = node.Leaves.Count;
        for (int i = 1; i < numberOfLeaves; i++)
          node.BoundingBox.Grow(node.Leaves[i].BoundingBox);
      }
    }
    ...
    private static void SplitIfNecessary(Node node)
    {
      ...
      else
      {
        ...
        if (leftleaves > subtreeThreshold || rightLeaves > subtreeThreshold)
        {
          ...
          if (leftleaves > rightLeaves)
          {
            ...
            splitValue = (node.BoundingBox.Min.GetComponentByIndex(splitAxis) + splitValue) / 2;
          }
          else
          {
            // Move split-plane to midpoint of right sub-volume.
            splitValue = (node.BoundingBox.Max.GetComponentByIndex(splitAxis) + splitValue) / 2;
          }
      ...
      else
      {
        ...
        child.Leaves = DigitalRise.ResourcePools<Node>.Lists.Obtain();
        Node leaf = node.Leaves[0];
        child.Leaves.Add(leaf);
        child.BoundingBox = leaf.BoundingBox;
        for (int index = 1; index < rightLeaf; index++)
        {
          leaf = node.Leaves[index];
          child.Leaves.Add(leaf);
          child.BoundingBox.Grow(leaf.BoundingBox);
        }
      ...
      DigitalRise.ResourcePools<Node>.Lists.Recycle(node.Leaves);
      node.Leaves = null;
