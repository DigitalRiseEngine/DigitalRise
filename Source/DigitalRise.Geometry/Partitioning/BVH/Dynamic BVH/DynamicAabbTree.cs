...
namespace DigitalRise.Geometry.Partitioning
{
  ...
  public partial class DynamicBoundingBoxTree<T> : BasePartition<T>, ISupportClosestPointQueries<T>, ISupportFrustumCulling<T>
  {
    ...
    internal override void OnUpdate(bool forceRebuild, HashSet<T> addedItems, HashSet<T> removedItems, HashSet<T> invalidItems)
    {
      ...
      List<Node> invalidNodes = (trackInvalidNodes) ? DigitalRise.ResourcePools<Node>.Lists.Obtain() : null;
      ...
      if (invalidNodes != null)
        DigitalRise.ResourcePools<Node>.Lists.Recycle(invalidNodes);
    }
    ...
    private void UpdateSelfOverlaps(HashSet<T> addedItems, HashSet<T> removedItems, HashSet<T> invalidItems, List<Node> invalidNodes)
    {
      ...
      else
      {
        ...
        {
          var invalidOverlaps = DigitalRise.ResourcePools<Pair<T>>.Lists.Obtain();
          ...
          DigitalRise.ResourcePools<Pair<T>>.Lists.Recycle(invalidOverlaps);
        }
