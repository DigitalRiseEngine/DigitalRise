...
namespace DigitalRise.Geometry.Partitioning
{
  partial class DynamicBoundingBoxTree<T>
  {
    ...
    public IEnumerable<T> GetOverlaps(IList<Plane> planes)
    {
      ...
      var stack0 = DigitalRise.ResourcePools<Pair<Node, int>>.Stacks.Obtain();
      var stack1 = DigitalRise.ResourcePools<Node>.Stacks.Obtain();
      var signs = DigitalRise.ResourcePools<int>.Lists.Obtain();
      ...
      DigitalRise.ResourcePools<Pair<Node, int>>.Stacks.Recycle(stack0);
      DigitalRise.ResourcePools<Node>.Stacks.Recycle(stack1);
      DigitalRise.ResourcePools<int>.Lists.Recycle(signs);
#else
      if (_root == null || planes == null)
