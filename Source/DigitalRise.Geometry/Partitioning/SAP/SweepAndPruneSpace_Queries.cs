...
namespace DigitalRise.Geometry.Partitioning
{
  partial class SweepAndPruneSpace<T>
  {
    ...
    public override IEnumerable<T> GetOverlaps(BoundingBox aabb)
    {
      ...
      var overlapCandidates = DigitalRise.ResourcePools<ItemInfo>.HashSets.Obtain();
      ...
      DigitalRise.ResourcePools<ItemInfo>.HashSets.Recycle(overlapCandidates);        
#else
      // Avoiding garbage:
    ...
    private sealed class GetOverlapsWork : PooledEnumerable<T>
    {
      ...
      {
        DigitalRise.ResourcePools<ItemInfo>.HashSets.Recycle(_candidates);
        Pool.Recycle(this);
      }
    }
#endif
