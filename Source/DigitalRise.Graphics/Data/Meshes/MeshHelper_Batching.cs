	...
	partial class MeshHelper
	{
				...
				foreach (var meshNode in meshNodes)
				{
					...
					foreach (var submesh in mesh.Submeshes)
					{
						if (submesh.PrimitiveCount <= 0)
							continue;
						if (submesh.VertexBuffer == null || submesh.Material == null)
							continue;

						// Merge materials and get material index.
						int mergedMaterialIndex = materials.IndexOf(submesh.Material);
						if (mergedMaterialIndex < 0)
						{
							mergedMaterialIndex = materials.Count;
		...
		public static Mesh Merge(Mesh mesh, IList<Vector3> scales, IList<Pose> poses)
		{
			...
			try
			{
				...
				foreach (var submesh in mesh.Submeshes)
				{
					...
					// Merge materials and get material index.
					var mergedMaterialIndex = materials.IndexOf(submesh.Material);
					if (mergedMaterialIndex < 0)
					{
						mergedMaterialIndex = materials.Count;
						materials.Add(submesh.Material);
										   ...
										   List<int> totalVertexCounts, int totalIndexCount)
		{
			...
			for (int jobIndex = 0; jobIndex < mergeJobs.Count; jobIndex++)
			{
				...
				{
					sortKey = job.SortKey;
					submesh = new Submesh(vertexBuffer, (job.Submesh.IndexBuffer != null) ? indexBuffer : null, submesh.BoundingBox, job.Submesh.PrimitiveType)
					{
						StartVertex = vertexCount,
						StartIndex = indexCount,
						Material = job.Submesh.Material,
					};
