...
{
	internal class BlurEffectBinding : EffectWrapper
	{
		public EffectParameter ViewportSize { get; private set; }
		public EffectParameter Weights { get; private set; }
		public EffectParameter SourceTexture { get; private set; }
		public EffectParameter Offsets { get; private set; }

		public EffectParameter GBuffer0 { get; private set; }
		public EffectParameter FrustumCorners { get; private set; }
		public EffectParameter BlurParameters0 { get; private set; }

		public BlurEffectBinding() : base("PostProcessing/Blur")
		{
		}
		...
		protected override void BindParameters(Effect effect)
		{
			...
			Offsets = effect.Parameters["Offsets"];

			GBuffer0 = effect.Parameters["GBuffer0"];
			FrustumCorners = effect.Parameters["FrustumCorners"];
			BlurParameters0 = effect.Parameters["BlurParameters0"];
		}
	}
	...
	public class Blur : PostProcessor
	{
		...
		private readonly BlurEffectBinding _effect = new BlurEffectBinding();
		...
		protected override void OnProcess(RenderContext context)
		{
			...
			else
			{
				if (isAnisotropic || isBilateral)
				{
					...
					float far = cameraNode.ViewVolume.Far;

					GraphicsHelper.GetFrustumFarCorners(cameraNode.ViewVolume, _frustumFarCorners);
					_effect.FrustumCorners.SetValue(_frustumFarCorners);

					_effect.BlurParameters0.SetValue(new Vector4(
					  far,
					...
					if (viewport.Width < depthBuffer.Width && viewport.Height < depthBuffer.Height)
					{
						...
						if (context.DepthBufferHalf != null)
						{
							depthBuffer = context.DepthBufferHalf;
						}
					}

					_effect.GBuffer0.SetValue(depthBuffer);
				}
			}

			_effect.ViewportSize.SetValue(size);
			_effect.Weights.SetValue(Weights);
			...
			for (int i = 0; i < effectiveNumberOfPasses; i++)
			{
				...
				{
					context.RenderTarget = tempTarget0;
				}
				else
				{
					context.RenderTarget = tempTarget1;
				}
				...
				{
					_effect.SourceTexture.SetValue(tempTarget1);
				}
				else
				{
					_effect.SourceTexture.SetValue(tempTarget0);
				}
				...
				_effect.Offsets.SetValue(offsets);

				int passIndex = 0;
				if (isAnisotropic)
					passIndex = i % 2;

				context.DrawFullScreenQuad(_effect.CurrentTechnique.Passes[passIndex]);
			}
