...
namespace DigitalRise.Rendering.Billboards
{
	...
	internal abstract class BillboardBatch<T> : IBillboardBatch where T : struct, IVertexType
	{
		...
		/// <inheritdoc/>
		public virtual void Begin(CameraNode camera)
		{
		}


		/// <inheritdoc/>
		public virtual void End(ref RenderStatistics statistics)
		{
			_renderBatch.Flush(ref statistics);
		}


		/// <inheritdoc/>
		public void DrawBillboard(ref BillboardArgs billboard, PackedTexture texture, ref RenderStatistics statistics)
		{
			int index, dummy;
			_renderBatch.Submit(PrimitiveType.TriangleList, 4, 6, out index, out dummy, ref statistics);
		...
		/// <inheritdoc/>
		public void DrawRibbon(ref RibbonArgs p0, ref RibbonArgs p1, PackedTexture texture, ref RenderStatistics statistics)
		{
			int index, dummy;
			_renderBatch.Submit(PrimitiveType.TriangleList, 4, 6, out index, out dummy, ref statistics);
