...
{
	internal class BillboardEffectWrapper : EffectWrapper
	{
		public EffectParameter View { get; private set; }
		public EffectParameter ViewInverse { get; private set; }
		public EffectParameter ViewProjection { get; private set; }
		...
		public EffectTechnique TechniqueHardLinear { get; private set; }
		public EffectTechnique TechniqueHardGamma { get; private set; }
		...
		protected override void BindParameters(Effect effect)
		{
			...
			TechniqueHardLinear = effect.Techniques["HardLinear"];
			TechniqueHardGamma = effect.Techniques["HardGamma"];
			TechniqueSoftLinear = effect.Techniques["SoftLinear"];
			TechniqueSoftGamma = effect.Techniques["SoftGamma"];
		}
	}
	...
	/// </note>
	/// </remarks>
	public partial class BillboardRenderer : IDisposable
	{
		// Notes: 
		...
		private readonly List<Job> _jobs = new List<Job>(32);
		...
		/// </exception>
		public BillboardRenderer(int bufferSize)
		...
		/// <paramref name="bufferSize"/> is 0, negative, or greater than <see cref="MaxBufferSize"/>.
		/// </exception>
		public BillboardRenderer(int bufferSize, SpriteFontBase spriteFont)
		{
		...
		public void Render(RenderContext context, IList<SceneNode> nodes, RenderOrder order)
		{
			ThrowIfDisposed();
		...
		private void ProcessJobs(RenderContext context)
		{
			...
			for (var index = 0; index < _jobs.Count;)
			{
				uint textureId = _jobs[index].TextureId;

				int endIndex = index + 1;
				while (endIndex < _jobs.Count && _jobs[endIndex].TextureId == textureId)
					endIndex++;
