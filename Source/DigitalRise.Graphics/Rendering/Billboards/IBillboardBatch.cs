...
namespace DigitalRise.Rendering.Billboards
{
	...
	interface IBillboardBatch : IDisposable
	{
		...
		void Begin(CameraNode cameraNode);


		/// <summary>
		/// Submits the current batch.
		/// </summary>
		void End(ref RenderStatistics statistics);
		...
		void DrawBillboard(ref BillboardArgs billboard, PackedTexture texture, ref RenderStatistics statistics);
		...
		/// <param name="texture">The packed texture.</param>
		void DrawRibbon(ref RibbonArgs p0, ref RibbonArgs p1, PackedTexture texture, ref RenderStatistics statistics);
	}
}
