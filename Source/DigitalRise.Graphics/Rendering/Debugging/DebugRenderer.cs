	...
	public class DebugRenderer : IDisposable
	{
		...
		private TextBatch InSceneTextBatch
		{
			get
			{
				...
				{
					_inSceneTextBatch = new TextBatch(SpriteFont)
					{
		...
		private TextBatch OverScene2DTextBatch
		{
			get
			{
				...
				{
					_overScene2DTextBatch = new TextBatch(SpriteFont)
					{
		...
		private TextBatch OverScene3DTextBatch
		{
			get
			{
				...
				{
					_overScene3DTextBatch = new TextBatch(SpriteFont)
					{
						EnableDepthTest = false
					};
				}
		...
		private PrimitiveBatch OpaquePrimitiveBatch
		{
			get
			{
				...
				{
					_opaquePrimitiveBatch = new PrimitiveBatch(Effect)
					{
						DrawWireFrame = false,
		...
		private PrimitiveBatch TransparentPrimitiveBatch
		{
			get
			{
				...
				{
					_transparentPrimitiveBatch = new PrimitiveBatch(Effect)
					{
						DrawWireFrame = false,
		...
		private PrimitiveBatch InSceneWireFramePrimitiveBatch
		{
			get
			{
				...
				{
					_inSceneWireFramePrimitiveBatch = new PrimitiveBatch(Effect)
					{
						DrawWireFrame = true,
						SortBackToFront = false,
					};
		...
		private PrimitiveBatch OverSceneWireFramePrimitiveBatch
		{
			get
			{
				...
				{
					_overSceneWireFramePrimitiveBatch = new PrimitiveBatch(Effect)
					{
						DrawWireFrame = true,
						SortBackToFront = false,
					};
		...
		private TextureBatch TextureBatch
		{
			get
			{
				if (_textureBatch == null)
					_textureBatch = new TextureBatch();
		...
		public BasicEffect Effect
		{
			get
			{
				if (_effect == null)
				{
					_effect = new BasicEffect(DR.GraphicsDevice)
					{
						FogEnabled = false,
		...
		public DebugRenderer(SpriteFontBase SpriteFontBase)
		{
		...
		public DebugRenderer() : this(Resources.DefaultFont)
		{
		}
      ...
      skipWireframes = true;
			...
			if (_inSceneLineBatch != null)
				_inSceneLineBatch.Render(cameraNode);

			if (_inSceneWireFramePrimitiveBatch != null && !skipWireframes)
				_inSceneWireFramePrimitiveBatch.Render(cameraNode);
			...
			if (_inSceneWireFrameTriangleBatch != null && !skipWireframes)
				_inSceneWireFrameTriangleBatch.Render(cameraNode);

			if (_autoRenderStates)
				graphicsDevice.RasterizerState = GraphicsHelper.RasterizerStateCullCounterClockwise;

			if (_inScenePointBatch != null)
				_inScenePointBatch.Render(cameraNode);

			if (_inSceneTextBatch != null)
				_inSceneTextBatch.Render(cameraNode);
			...
			if (_opaquePrimitiveBatch != null)
				_opaquePrimitiveBatch.Render(cameraNode);

			if (_opaqueTriangleBatch != null)
				_opaqueTriangleBatch.Render(cameraNode);
			...
			if (_transparentPrimitiveBatch != null)
				_transparentPrimitiveBatch.Render(cameraNode);

			if (_transparentTriangleBatch != null)
				_transparentTriangleBatch.Render(cameraNode);
			...
			if (_overSceneWireFramePrimitiveBatch != null && !skipWireframes)
				_overSceneWireFramePrimitiveBatch.Render(cameraNode);

			if (_autoRenderStates)
				graphicsDevice.RasterizerState = GraphicsHelper.RasterizerStateWireFrame;

			if (_overSceneWireFrameTriangleBatch != null && !skipWireframes)
				_overSceneWireFrameTriangleBatch.Render(cameraNode);

			if (_autoRenderStates)
				graphicsDevice.RasterizerState = GraphicsHelper.RasterizerStateCullCounterClockwise;

			if (_overSceneLineBatch != null)
				_overSceneLineBatch.Render(cameraNode);

			if (_overScenePointBatch != null)
				_overScenePointBatch.Render(cameraNode);

			if (_overScene3DTextBatch != null)
				_overScene3DTextBatch.Render(cameraNode);

			if (_textureBatch != null)
				_textureBatch.Render();

			if (_overScene2DTextBatch != null)
				_overScene2DTextBatch.Render(cameraNode);
		...
		/// space axes (x, y, z).
		/// </remarks>
		public void DrawSkeleton(DrModelNode skeletonPose, Pose pose, Vector3 scale, float axisLength, Color color, bool drawOverScene)
		{
			...
			foreach(var bone in skeletonPose.Model.Bones)
			{
				string name = bone.Name;
				SrtTransform bonePose = new SrtTransform(skeletonPose.GetBoneGlobalTransform(bone.Index));
				...
				if (bone.Parent != null)
				{
					...
					SrtTransform parentPose = new SrtTransform(skeletonPose.GetBoneGlobalTransform(bone.Parent.Index));
				...
				textBatch.Add(
				  string.Format(CultureInfo.InvariantCulture, "{0} {1}", name, bone.Index),
