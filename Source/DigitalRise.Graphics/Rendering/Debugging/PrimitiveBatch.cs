...
namespace DigitalRise.Rendering.Debugging
{
	...
	internal sealed class PrimitiveBatch
	{
		...
		/// </exception>
		public PrimitiveBatch(BasicEffect effect)
		{
			Effect = effect;
			...
			foreach (var job in _primitives)
			{
				...
				switch (job.Type)
				{
					...
					case PrimitiveJobType.ViewVolume:
						RenderViewVolume(job, cameraNode);
						break;
					case PrimitiveJobType.Sphere:
						RenderSphere(job, cameraNode);
						break;
		...
		private void RenderBox(PrimitiveJob job)
		{
			...
			if (DrawWireFrame)
			{
				if (_boxLinePrimitive == null)
					_boxLinePrimitive = MeshPrimitives.CreateBoxLinesSubmesh();
			...
			else
			{
				if (_boxPrimitive == null)
					_boxPrimitive = MeshPrimitives.CreateBoxSubmesh(Vector3.One);
		...
		private void RenderViewVolume(PrimitiveJob job, CameraNode cameraNode)
		{
			...
			else
			{
				...
				_triangleBatch.Render(cameraNode);
				_triangleBatch.Clear();
			}
		}


		private void RenderSphere(PrimitiveJob job, CameraNode cameraNode)
		{
			if (DrawWireFrame)
			{
				...
				if (Numeric.AreEqual(1.0f, cameraProjection.M33))
				{
					...
					if (_circleLinePrimitive == null)
						_circleLinePrimitive = MeshPrimitives.CreateCircleLinesSubmesh();
			...
			else
			{
				...
				if (_icospherePrimitive == null)
					_icospherePrimitive = MeshPrimitives.CreateSphereSubmesh();
		...
		private void RenderCapsule(PrimitiveJob job)
		{
			...
			if (DrawWireFrame)
			{
				if (_hemisphereLinePrimitive == null)
					_hemisphereLinePrimitive = MeshPrimitives.CreateHemisphereLinesSubmesh();
			...
			else
			{
				if (_hemispherePrimitive == null)
					_hemispherePrimitive = MeshPrimitives.CreateHemisphereSubmesh();
				...
				if (_uncappedCylinderPrimitive == null)
					_uncappedCylinderPrimitive = MeshPrimitives.CreateCylinderSubmesh(capped: false);
		...
		private void RenderCone(PrimitiveJob job)
		{
			float radius = job.Size.Radius;
			float height = job.Size.Height;
			...
			if (DrawWireFrame)
			{
				if (_coneLinePrimitive == null)
					_coneLinePrimitive = MeshPrimitives.CreateConeLinesSubmesh();
			...
			else
			{
				if (_conePrimitive == null)
					_conePrimitive = MeshPrimitives.CreateConeSubmesh();
		...
		private void RenderCylinder(PrimitiveJob job)
		{
			float radius = job.Size.Radius;
			float height = job.Size.Height;
			...
			if (DrawWireFrame)
			{
				if (_cylinderLinePrimitive == null)
					_cylinderLinePrimitive = MeshPrimitives.CreateCylinderLinesSubmesh();
			...
			else
			{
				if (_cylinderPrimitive == null)
					_cylinderPrimitive = MeshPrimitives.CreateCylinderSubmesh();
		...
		private void RenderShape(GraphicsDevice graphicsDevice, PrimitiveJob job)
		{
			Submesh submesh;
			Matrix44F matrix;
			ShapeMeshCache.GetSubmesh(job.Shape, out submesh, out matrix);
