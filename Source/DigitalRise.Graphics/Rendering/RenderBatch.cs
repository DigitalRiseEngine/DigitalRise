...
namespace DigitalRise.Rendering
{
	...
	internal sealed class RenderBatch<TVertex, TIndex> : IDisposable where TVertex : struct where TIndex : struct
	{
		...
		public void Submit(PrimitiveType primitiveType, int newVertices, int newIndices,
		  out int nextVertexBufferIndex, out int nextIndexBufferIndex, ref RenderStatistics statistics)
		{
		...
		/// </summary>
		public void Flush(ref RenderStatistics statistics)
		{
			...
			if (numberOfVertices > 0 && numberOfIndices > 0)
			{
				...
				statistics.PrimitivesDrawn += numberOfPrimitives;
				statistics.VerticesDrawn += numberOfVertices;
