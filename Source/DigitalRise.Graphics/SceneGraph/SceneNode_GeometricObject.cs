...
namespace DigitalRise.SceneGraph
{
	partial class SceneNode
	{
		...
		public Vector3 RotationLocal
		{
			...
			set
			{
				if (Mathematics.MathHelper.AreNumericallyEqual(_rotationLocal, value))
				{
					return;
				}
		...
		private Vector3 _rotationLocal;
		...
		public Vector3 PositionLocal
		{
			get
			{
				return _poseLocal.Position;
			}

			set
			{
				if (Mathematics.MathHelper.AreNumericallyEqual(_poseLocal.Position, value))
				{
					return;
				}

				_poseLocal.Position = value;
				Invalidate();
			}
		}
		...
		/// <see cref="DigitalRise.Geometry.Shapes.Shape.Empty"/>.
		/// </para>
		/// <para>
		/// Changing this property raises the <see cref="IGeometricObject.ShapeChanged"/> event.
		/// </para>
		...
		/// contained <see cref="DigitalRise.Geometry.Shapes.Shape"/>. Therefore, a 
		/// <see cref="DigitalRise.Geometry.Shapes.Shape"/> will have an indirect reference to the 
		...
		/// <see cref="DigitalRise.Geometry.Shapes.Shape.Empty"/> or 
		/// <see cref="DigitalRise.Geometry.Shapes.Shape.Infinite"/> when the 
		/// <see cref="IGeometricObject"/> is no longer used. Those are special immutable shapes that 
		/// never raises any <see cref="DigitalRise.Geometry.Shapes.Shape.Changed"/> events. Setting 
		/// the shape to <see cref="DigitalRise.Geometry.Shapes.Shape.Empty"/> or 
		/// <see cref="DigitalRise.Geometry.Shapes.Shape.Infinite"/> ensures that the internal event 
		...
		public Matrix CalculateGlobalTransform() => PoseWorld.ToMatrixWithScale(ScaleWorld);
		#endregion
	}
}
