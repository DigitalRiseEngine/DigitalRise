...
namespace DigitalRise.Animation.Tests.Animations.NBlendAnimation
{
	...
	public class BlendGroupTest
	{
		...
		public void BlendGroupWithTwoAnimationsSynchronized()
		{
			...
			blendGroup.Add(animation1);
			blendGroup.Add(animation2);
			AssertExt.AreNumericallyEqual(1.0f, blendGroup.GetWeight(0));
			AssertExt.AreNumericallyEqual(1.0f, blendGroup.GetWeight(1));
			AssertExt.AreNumericallyEqual(TimeSpan.FromSeconds(2.0), blendGroup.GetTotalDuration());

			blendGroup.SynchronizeDurations();
			AssertExt.AreNumericallyEqual(TimeSpan.FromSeconds(1.5), blendGroup.GetTotalDuration());

			var manager = new AnimationManager();
			var controller = manager.StartAnimation(blendGroup, property1);
			controller.UpdateAndApply();
			AssertExt.AreNumericallyEqual(50.0f, property1.Value);

			manager.Update(TimeSpan.FromSeconds(0.75));      // t = 0.75
			manager.ApplyAnimations();
			AssertExt.AreNumericallyEqual(TimeSpan.FromSeconds(1.5), blendGroup.GetTotalDuration());
			AssertExt.AreNumericallyEqual(0.5f * 50.0f + 0.5f * 200.0f, property1.Value);

			blendGroup.SetWeight(0, 0);
			AssertExt.AreNumericallyEqual(TimeSpan.FromSeconds(2.0), blendGroup.GetTotalDuration());
			manager.Update(TimeSpan.FromSeconds(0.25));       // t = 1.0
			manager.ApplyAnimations();
			AssertExt.AreNumericallyEqual(200.0f, property1.Value);

			blendGroup.SetWeight(0, 10);
			blendGroup.SetWeight(1, 0);
			AssertExt.AreNumericallyEqual(TimeSpan.FromSeconds(1.0), blendGroup.GetTotalDuration());
			manager.Update(TimeSpan.Zero);       // t = 1.0
			manager.ApplyAnimations();
			AssertExt.AreNumericallyEqual(100.0f, property1.Value);
			...
			manager.ApplyAnimations();
			AssertExt.AreNumericallyEqual(123.45f, property1.Value);
			Assert.AreEqual(AnimationState.Stopped, controller.State);
		}
