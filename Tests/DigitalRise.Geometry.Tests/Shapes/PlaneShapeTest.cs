...
namespace DigitalRise.Geometry.Shapes.Tests
{
  ...
  public class PlaneShapeTest
  {
    ...
    public void GetAxisAlignedBoundingBox()
    {
      ...
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, nInf, nInf), new Vector3(pInf, 0, pInf)), new PlaneShape().GetBoundingBox(Pose.Identity));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf), new Vector3(pInf)),
                     new PlaneShape().GetBoundingBox(new Pose(new Vector3(10, 100, -13),
                                                                         MathHelper.CreateRotation(new Vector3(1, 1, 1), 0.7f))));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, nInf, nInf), new Vector3(12, pInf, pInf)),
                      new PlaneShape(new Vector3(1, 0, 0), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      Assert.AreEqual(new BoundingBox(new Vector3(8, nInf, nInf), new Vector3(pInf, pInf, pInf)),
                      new PlaneShape(new Vector3(-1, 0, 0), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, nInf, nInf), new Vector3(pInf, 102, pInf)),
                      new PlaneShape(new Vector3(0, 1, 0), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, 98, nInf), new Vector3(pInf, pInf, pInf)),
                      new PlaneShape(new Vector3(0, -1, 0), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, nInf, nInf), new Vector3(pInf, pInf, 1002)),
                      new PlaneShape(new Vector3(0, 0, 1), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      Assert.AreEqual(new BoundingBox(new Vector3(nInf, nInf, 998), new Vector3(pInf, pInf, pInf)),
                      new PlaneShape(new Vector3(0, 0, -1), 2).GetBoundingBox(new Pose(new Vector3(10, 100, 1000), Quaternion.Identity)));
      // TODO: Test rotations.
    }
    ...
    public void Clone()
    {
      ...
      Assert.AreEqual(plane.GetBoundingBox(Pose.Identity).Min, clone.GetBoundingBox(Pose.Identity).Min);
      Assert.AreEqual(plane.GetBoundingBox(Pose.Identity).Max, clone.GetBoundingBox(Pose.Identity).Max);
    }
